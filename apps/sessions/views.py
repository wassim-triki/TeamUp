...
from apps.feedback.models import SessionFeedback  # Optional caching for AI
...
def session_list(request):
    ...
    if request.user.is_authenticated:
        ...
        queryset = (
            ...
            .prefetch_related("invitation_set__invitee")
        ...
        invited_session_ids = set(
            Invitation.objects.filter(invitee=request.user).values_list(
                "session_id", flat=True
            )
        )
        is_creator_ids = set(
            Session.objects.filter(creator=request.user).values_list("id", flat=True)
        )
...
def session_detail(request, pk):
    ...
    if not request.user.is_authenticated and session.status not in [
        "proposed",
        "confirmed",
    ]:
    ...
    responses_count = session.invitation_set.filter(
        status__in=["accepted", "refused", "rescheduled"]
    ).count()
    responses_with_notes = (
        session.invitation_set.filter(response_notes__isnull=False)
    ...
    if request.method == "POST" and request.user.is_authenticated:
        if session.creator == request.user:
            ...
            messages.success(
                request, "Join request sent! The creator will be notified."
            )
...
def request_join(request, pk):
    ...
    if (
        ...
        or session.invitation_set.filter(invitee=request.user).exists()
    ):
...
def create_session(request):
    ...
    return render(
        ...
        {"form": form, "title": "Create Session", "submit_text": "Create Session"},
    )
...
def invite_users(request, pk):
    ...
    excluded_users = [request.user.id] + list(
        session.invitation_set.values_list("invitee_id", flat=True)
    )
        ...
        if form.is_valid():
            ...
            for user in selected_users:
                invitation, created_new = Invitation.objects.get_or_create(
                    session=session, invitee=user, defaults={"status": "pending"}
                )
                if created_new:
                    created_count += 1
    ...
    return render(
        request,
        "sessions/invite.html",
        {"session": session, "form": form, "available_users": available_users},
    )
...
def ai_insight(request, pk):
    ...
    if raw_insights and not raw_insights.startswith(
        ("No insights", "Failed", "Gemini returned")
    ):
        insights_html = markdown(raw_insights, extensions=["extra", "fenced_code"])
    else:
        insights_html = None
        messages.warning(
            request, raw_insights or "AI generation failed—try richer session details."
        )
...
def manage_invitation(request, invitation_id):
    ...
    if request.method == "POST":
        ...
        if action == "accept":
            ...
            messages.success(
                request,
                f"{invitation.invitee.username} has been accepted to the session.",
            )
...
class SessionDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(
            self.request,
            f'Session "{session.get_sport_type_display()}" deleted. All invites have been declined.',
        )
